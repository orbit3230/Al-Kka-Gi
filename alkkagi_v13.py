...
def train() :
    ...
    log_filename = "training_log_v13.csv"
    ...
    for episode in range(episodes) :
        ...
        if((episode + 1) % 10000 == 0) :  # temporary save
            black_agent.save(f"./moka_black_v13_{episode + 1}")
            white_agent.save(f"./moka_white_v13_{episode + 1}")
    
    black_agent.save("./moka_black_v13")
    white_agent.save("./moka_white_v13")
    env.close()
...
def test() :
    ...
    black_agent = BlackAgent.load("./moka_black_v13")
    white_agent = WhiteAgent.load("./moka_white_v13")
    for _ in range(10) :    
        observation, info = env.reset()
        done = False
        while not done :
