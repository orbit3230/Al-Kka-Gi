...
# ---------- Training & Testing ----------
def train(resume_path = None, start_episode = 0) :
    # Environment
    env = gym.make(
    ...
    if(resume_path) :
        ...
        actor_model = keras.models.load_model(resume_path + "_actor.keras", safe_mode=False)
        critic_model = keras.models.load_model(resume_path + "_critic.keras", safe_mode=False)
    else :
        print("Starting training from scratch ...")
        actor_model, critic_model = build_actor_critic_model((30, ))
    ...
    log_filename = "training_log_v14.csv"
    file_mode = 'a' if resume_path and os.path.exists(log_filename) else 'w'
    ...
    for episode in range(start_episode, start_episode + episodes) :
        ...
        step_penalty = (1-GAMMA) * abs(draw_penalty)  # modified in v14
        time_over = False 
        ...
        while not done :
            ...
            if(turn == 0) :
                ...
                norm_power = (action["power"] - min_power) / (max_power - min_power)
                norm_power = float(np.clip(norm_power, 0.0, 1.0))
                original_angle = 180.0 - action["angle"]
                if(original_angle > 180.0) : original_angle -= 360.0
                if(original_angle < -180.0) : original_angle += 360.0
                norm_angle = float(np.clip(original_angle / 180.0, -1.0, 1.0))  # Normalize back to (-1.0 ~ 1.0)
                raw_action = [action["index"], norm_power, norm_angle]
        ...
        if(winner == "black") :
            ...
            black_reward = draw_penalty
            white_reward = draw_penalty
        ...
        if(black_training_phase) :
            if(len(black_records["states"]) > 0) :
                ...
                loss_black, critic_loss_black, entropy_black = train_by_records(black_agent, np.array(black_records["states"]), np.array(black_records["masks"]), np.array(black_records["actions"]), black_returns, actor_optimizer, critic_optimizer)
        if(white_training_phase) :
            if(len(white_records["states"]) > 0) :
                ...
                loss_white, critic_loss_white, entropy_white = train_by_records(white_agent, np.array(white_records["states"]), np.array(white_records["masks"]), np.array(white_records["actions"]), white_returns, actor_optimizer, critic_optimizer)
        ...
        if((episode + 1) % 10000 == 0) :  # temporary save
            black_agent.save(f"./moka_black_v14_{episode + 1}")
            white_agent.save(f"./moka_white_v14_{episode + 1}")
    
    black_agent.save("./moka_black_v14")
    white_agent.save("./moka_white_v14")
    env.close()
...
def test() :
    ...
    black_agent = BlackAgent.load("./moka_black_v14")
    white_agent = WhiteAgent.load("./moka_white_v14")
    for _ in range(10) :    
        observation, info = env.reset()
        done = False
        while not done :
...
if __name__ == "__main__" :
    ...
    train("./moka_v14", start_episode=130000)
    test()