...
def collision_detected(observation_before, observation_after) :
    ...
    for x_b_before, y_b_before, x_b_after, y_b_after in zip(black_x_before, black_y_before, black_x_after, black_y_after) :
        if(abs(x_b_before - x_b_after) > 5.0 or abs(y_b_before - y_b_after) > 5.0) :
    ...
    for x_w_before, y_w_before, x_w_after, y_w_after in zip(white_x_before, white_y_before, white_x_after, white_y_after) :
        if(abs(x_w_before - x_w_after) > 5.0 or abs(y_w_before - y_w_after) > 5.0) :
...
def train(resume_path_black = None, resume_path_white = None, start_episode = 0) :
    ...
    log_filename = "training_log_v15.csv"
    ...
    for episode in range(start_episode, start_episode + episodes) :
        ...
        while not done :
            ...
            if(turn == 0) :
                black_records["rewards"][-1] -= capture_reward_black  # self dying penalty
                black_records["rewards"][-1] += capture_reward_white  # opponent capture reward
            else :
                white_records["rewards"][-1] -= capture_reward_white  # self dying penalty
                white_records["rewards"][-1] += capture_reward_black  # opponent capture reward
        ...
        if((episode + 1) % 10000 == 0) :  # temporary save
            black_agent.save(f"./moka_black_v15_{episode + 1}")
            white_agent.save(f"./moka_white_v15_{episode + 1}")
    
    black_agent.save("./moka_black_v15")
    white_agent.save("./moka_white_v15")
    env.close()
