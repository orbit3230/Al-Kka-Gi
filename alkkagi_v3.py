...
def get_min_distance(observation, turn) :
    if(turn == 0) :
    ...
    players = [stone for stone in players if stone[2] == 1]
    opponents = [stone for stone in opponents if stone[2] == 1]
    
    if(not players or not opponents) : return 0.0
...
def train() :
    ...
    for episode in range(episodes) :
        ...
        old_black_distance = get_min_distance(observation, 0)
        old_white_distance = get_min_distance(observation, 1)
        new_black_distance = old_black_distance
        new_white_distance = old_white_distance
        ...
        while not done :
            ...
            old_black_distance = new_black_distance
            ...
            distance_reward_black = np.clip(distance_reward_black, -0.1, 0.1)
            distance_reward_white = np.clip(distance_reward_white, -0.1, 0.1)
    ...
    black_agent.save("./moka_black_v2.keras")
    white_agent.save("./moka_white_v2.keras")
    env.close()
...
def test() :
    ...
    black_agent = BlackAgent.load("./moka_black_v2.keras")
    white_agent = WhiteAgent.load("./moka_white_v2.keras")
    for _ in range(10) :    
        observation, info = env.reset()
        done = False
        while not done :
